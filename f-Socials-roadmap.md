...
Principle: **trust surfaces before account surfaces.** The engine being done made this fast. Status: the **trust-and-launch-bundle spec delivered the P0/P1 technical items below** (transparent source-tier policy, Methodology page, dispute/flag intake, production build + safe deploy config, and the UI/accessibility pass). The non-technical educator outreach and the evidence-parallelization perf item remain open.
...
- [ ] **P1 — Parallelize evidence lookups.** `pipeline/stages.ts` runs the claim-verification-router per claim in series, and the router now submits each Query_Variant to the chain sequentially (≤6 variants/claim) — so the per-claim API-call count went *up*, making this more relevant. Ship gate is p95 ≤ 30s on cache miss; a `Promise.all` with a concurrency cap (across claims and/or variants) protects it. *(not part of the trust bundle or the router spec)*
...
| Phase | Focus |
|---|---|
| **Pilots** | Normalize the report graph (§7) · expert review queue UI · institutional workspace (shared collections, classroom annotation) · accounts/save/history · EN/NL localization · EDMO/BENEDMO outreach for the human-review layer via partnership, not payroll |
...
- [ ] **P1 — Normalize the report graph (key next-phase debt).** Reports are stored as JSONB in `analysis_reports.data`; the `claims`/`citations`/`perspective_links` tables are reserved but unpopulated. Every Ground-News-like aggregate (blindspot clustering, "this claim appeared in N reports," topic pages) needs cross-report queries JSONB can't serve cheaply. **Dual-write:** keep JSONB as the lossless render payload, populate normalized rows for analytics. Do it *before* the first aggregate feature, not after.
...
> ~~Ship the **methodology page + source-tier fix**, deploy the public lens **safely**, wire **dispute/flag intake**~~ — **done.** Remaining: **talk to five educators**, deploy to Railway/Vercel, and parallelize evidence lookups. That converts a working, defensibly-trustworthy engine into a validated, publicly-running one. Everything else is earned after.
