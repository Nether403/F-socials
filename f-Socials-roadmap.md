...
- **The engine is real and proven end-to-end.** Gemini extraction · Supadata/Firecrawl transcripts · Google Fact Check → GDELT → Tavily evidence chain · bridging perspectives · Neon Postgres · Upstash Redis + BullMQ · Supabase JWT auth · per-key rate limiting · cache-first hashing · the invariant gate · a live React report page (claim drawers, interactive transcript highlighting, issue-frame spectrum, provenance footer, public share route). The **trust-and-launch bundle** then added the transparent source-tier policy, the Methodology page, dispute/flag intake, the accessibility pass, and the real production build. The **claim-verification-router** rebuilt Stage 3's evidence step into a precision pipeline (normalize → triage → query pack → retrieve → validate → outcome) wrapped around the same provider chain — citing only evidence that actually matches a claim, reporting an honest "no sufficient evidence found" otherwise, persisting a lossless per-claim audit record, guarding the invariant gate at worker boot, and shipping an offline False-Evidence-Rate benchmark + Ship_Gate. Then **report-graph-normalization** landed the committed dual-write (§7): every saved report now also projects into the normalized `claims`/`citations`/`perspective_links` tables for cross-report analytics, additively and best-effort, with a backfill for older JSONB-only reports — the data layer the first aggregate feature will sit on. Most recently, **parallel-evidence-lookups** removed the last serial bottleneck: Stage 3's claim loop and the router's Query_Variant loop now run under one shared per-report counting semaphore (configurable `CONCURRENCY_CAP`, default 4), bounded so concurrent provider submissions never exceed the cap — output ordering, error isolation, and the invariant gate are preserved by construction (verified, not weakened). Then **observability-instrumentation** wired Sentry + PostHog behind a single fail-open `Telemetry` port (zero keys ⇒ no-op, never blocking startup or the pipeline), with a pure `Redactor` + `Neutrality_Guard` at the emission boundary and the two red-line trust KPIs (Citation_Coverage, Model_Human_Agreement) now derivable — the observability prerequisite for pilots (§7). Most recently, **expert-review-queue** built the review **workflow** that intentionally lagged the dispute/flag intake: role-gated reviewers list/claim/release/resolve incoming disputes and flags through new `Repository` methods + migration `005` (additive columns on the existing tables) and a `#/review` Reviewer Console, feeding the report's existing `Provenance.reviewStatus` via a **derived-on-read** overlay that never rewrites the report — so the invariant gate is preserved by construction, the outcome vocabulary is framing/evidence-only (a static Neutrality_Check enforces it), and the offline-first path stays green. Then **progressive-disclosure-report-ui** closed the §8 UI/UX block: a presentation-layer-only rework of the live report page into a `SummaryLead` (TLDR + single most-important framing signal) above independent, keyboard-operable disclosure drawers, with the mockup's "Why included" / "Why this is here" rationale blocks and the three lens-aligned borrowings — a descriptive "covered from one angle" note (Ground News), spatial issue-frame chips (AllSides), and source-tier chips on sources (NewsGuard) — all descriptive, never a verdict, no creator rating, no schema change, and the invariant gate read but never touched. Most recently, **accounts-save-history** opened Slice 2 ("Pilots"): the web app now has a real **client-side auth flow** (sign up / in / out, session restore + token refresh, `Authorization: Bearer` on identity/mutating calls) behind an `AuthClient` seam over `@supabase/supabase-js`, plus **save + reverse-chronological history** of analyzed reports per signed-in reader — new `Repository` methods (memory + Postgres) and migration `006` (`reader_saved_reports`, keyed on the Supabase JWT subject), three `requireAuth`-gated routes, a `#/sign-in` panel and a `#/history` view, and the existing Flag/Save controls finally wired to a real session (no longer an unconditional prompt). Additive and offline-first (no Supabase config ⇒ graceful degrade), the invariant gate untouched, and no account/history surface or API response carrying a verdict or creator rating. Most recently, **institutional-workspace** extended that same identity + persistence pattern from an individual reader to a **group** — the §5 Pilots "institutional workspace" entry: an Authenticated Reader creates a Workspace, issues a redeemable Invite_Code a colleague redeems to join, curates analyzed reports into Shared_Collections, and attaches classroom Annotations, every operation `requireAuth`-gated and scoped to the requesting reader's Membership. Nineteen additive `Repository` methods (memory + Postgres), one additive migration (`007`: `workspaces`/`workspace_members`/`workspace_invites`/`shared_collections`/`collection_items`/`annotations`, keyed on the Supabase JWT subject `TEXT`), a single `loadMembership` authorization guard (404-before-403) over sixteen routes, and `#/workspaces` + `#/workspaces/:id` surfaces on the hand-rolled hash router — invariant gate untouched, lens-not-judge by construction (no workspace/collection/annotation surface or response carries a verdict or creator rating; an annotation is a reader-authored note, never converted into a rating). Then **en-nl-localization** internationalized the web client (English/Dutch via a header Language_Selector, bundled 232-key catalogs with compile-time parity, `Intl` date/number formatting, `<html lang>` synced) — presentation-layer only, no i18n library, no server change, neutrality enforced across both languages by a build-time denylist scan. Most recently, **intervention-and-scale** shipped the final roadmap phase (§5), gated on trust metrics + legal review: three individually-gated capabilities built onto the **unchanged** engine — a **Feed Friction Dial** (a public, lens-safe feed-overlay `GET /friction` route + the read-only Extension's friction module), a **public GraphQL API for institutions** (API-key-authenticated, per-key rate-limited, read-only over the normalized Report_Graph via graphql-js), and **creator pre-publish coaching** (an authenticated, advisory-only `POST /coaching` that persists nothing). Each capability is fronted by a per-capability **Trust_Gate** (Citation_Coverage + Model_Human_Agreement thresholds + a legal-review flag, re-evaluated live every request, fail-closed, **dark by default**), every outbound payload passes the existing `neutralityGuard` at the send boundary, and the work is read-mostly and additive — migration `008` is additive-only, new state is reached only through new `Repository` methods (memory + Postgres parity), no path calls `assembleReport`, and a CI diff guard fails the build if `core/assemble.ts` or `pipeline/stages.ts` is touched. All backed by a green test suite (server unit + property tests, web Vitest + property tests, plus build-smoke and degraded-controls integration checks).
...
**Explicitly NOT short-term:** ~~accounts/save/history (Slice 2, once repeat use is visible)~~ **shipped (accounts-save-history)** · ~~institutional workspace~~ **shipped (institutional-workspace)** · extension. The dispute/flag *intake* shipped first; the *review workflow* (expert review queue) has since shipped too (**expert-review-queue** — see §5/§7 and the debt ledger).
...
| Phase | Focus |
|---|---|
| **Pilots** | ~~Normalize the report graph (§7)~~ **done** · ~~observability before pilots (§7)~~ **done** · ~~expert review queue UI~~ **done (expert-review-queue)** · ~~accounts/save/history~~ **done (accounts-save-history)** · ~~institutional workspace (shared collections, classroom annotation)~~ **done (institutional-workspace)** · ~~EN/NL localization~~ **done (en-nl-localization)** · EDMO/BENEDMO outreach for the human-review layer via partnership, not payroll |
| **Read-only extension** | Surfaces *existing* reports on a page — no reranking. Earns trust on a feed surface before touching ranking. (The read-only Extension package is the prerequisite; its **friction module** shipped with intervention-and-scale.) |
| **Intervention & scale** *(gated on trust metrics + legal review)* | ~~Feed Friction Dial (Wedge B) · public/GraphQL API for institutions (a real secondary revenue line — the one strong B2B idea from the architecture doc) · creator pre-publish coaching~~ **done (intervention-and-scale)** — all three capabilities built onto the unchanged engine, each individually gated by a per-capability Trust_Gate (Citation_Coverage + Model_Human_Agreement thresholds + a legal-review flag) and **dark by default** until explicitly enabled per `.env`. Lens-safe at every outbound boundary (`neutralityGuard`), invariant gate untouched (CI diff guard), offline path preserved. |
...
| Ground News blindspot | Descriptive "covered from one angle" note on the perspectives tab | **Done** — `CoverageAngleNote` names the strong issue-frame pole(s), descriptive (never accusatory), perspectives-directive only when perspectives exist (progressive-disclosure-report-ui) |
...
| AllSides | Spatial issue-frame chips (`issueFrameLabel`) | **Done** — descriptive `IssueFrameChip` at report level + per perspective, never a verdict (progressive-disclosure-report-ui) |
...
- [x] **Observability before pilots.** **Shipped (observability-instrumentation).** Sentry (error monitoring) + PostHog (product analytics) wired behind a single `Telemetry` port in `infra/ports.ts`, selected in `compose.ts` (`selectTelemetry`) exactly like the other infra drivers. Additive and **fail-open**: zero keys ⇒ a frozen `No_Op_Telemetry` (never blocks startup or the pipeline); a telemetry fault is contained, never reaching the request path, the pipeline, or boot. PII/secrets are stripped by a pure `Redactor` and creator/truth verdicts withheld by a pure `Neutrality_Guard` at the emission boundary; the vendor SDKs live in one module only. The two **red-line trust KPIs are now derivable** — `citationCoverage` and `modelHumanAgreement` (pure functions in `core/kpi.ts`) — so they can finally be measured. Web analytics (`posthog-js`) is consent-gated and has no render side effects. `core/assemble.ts` is untouched; telemetry is emitted only after the pipeline returns, never on the gate path. 13 `fast-check` properties + example/boot-smoke tests, all green.
...
The live report page is functional; the gap to "accessible and inclusive for a diverse audience" is progressive disclosure and reduced cognitive load, not a redesign. **Closed by progressive-disclosure-report-ui** (presentation-layer only — no schema change, report read immutably, invariant gate untouched).

- [x] **Reuse the mockup's visual language, keep the live app's data wiring.** The richer prototype (`Design concepts/mockup designs/.../components/report/*`) was simplified into tabs in the live app. The live app is schema-correct and wired to real data — **kept it**; ported the mockup's calmer card styling + "Why included" / "Why this is here" rationale blocks (verbatim, from fields already in the report) onto it. Did **not** fork back to the prototype's mock shape (per the debt ledger).
- [x] **Progressive disclosure is the inclusivity lever.** Leads with the TLDR + the single most important framing signal (soft amber underline + adjacent text label); everything else sits behind independent, keyboard-operable expandable drawers, collapsed on first paint. A diverse, non-technical audience bounces off a wall of tabs/tiers but stays for one clear sentence with a "show me why."
- [x] **Borrow, mapped to the lens:** Ground News blindspot → "covered from one angle" note (descriptive, not accusatory) · AllSides → spatial frame chips (not verdicts) · NewsGuard → source tier/reliability chip in citations, never on creators. All shipped as descriptive presentation (`CoverageAngleNote`, `IssueFrameChip`, `SourceTierChip`).
- [x] **Accessibility as a first-class pass:** text labels beside every color signal · keyboard-navigable drawers/tabs · ARIA on AI tooltips · screen-reader-friendly rendering of the issue-frame chart · the accent standardized to `#0d9488`. Shipped in the trust-and-launch bundle and preserved/extended through the progressive-disclosure rework. **EN/NL localization shipped (en-nl-localization)** — the interface is fully presentable in English or Dutch with a single-click Language_Selector, `<html lang>` synchronized, and dates/numbers formatted per locale. Target **WCAG 2.2 AA** — automated ARIA + contrast-variable checks pass; full conformance still needs manual browser/AT review.
