...
- Monorepo under `app/`. Two packages:
  ...
  - `app/apps/web` — React 19 + Vite + Vitest. **Hash routing only** (`#/...`), no router dependency. Icons from `lucide-react` (already installed). **Fully localized** (EN/NL) via `src/i18n/` — React 19 context, bundled catalogs, no i18n library dependency.
...
- The web app is **fully localized** for English and Dutch (`app/apps/web/src/i18n/`). All UI_Chrome resolves through `t(...)` from bundled catalogs; Report_Content (claims, framing, citations) passes through verbatim. The Language_Selector is in the header beside the theme toggle. Locale-aware date/number formatting uses the browser's native `Intl` (no external service). Dutch translations are human-quality but **not yet reviewed by a native speaker** — a review pass is recommended before the EDMO/BENEDMO pilot.
- Saved reports **and institutional workspaces** key on the **Supabase JWT subject (`TEXT`)**, not the legacy `users(id)` UUID (the `reader_saved_reports` table and the `007` workspace tables deliberately follow the migration-005 subject convention). The one legacy seam that still keys on `users(id)` — the authenticated community **flag** (`flags.user_id`) — is now closed by **User_Sync**: the flag route ensures a local `users` row keyed to the JWT subject exists (idempotent `Repository.ensureLocalUser`, derived solely from verified claims, no Supabase round-trip) before the FK-bearing insert, so the FK resolves under both drivers. Additive migration `009_user_sync.sql` relaxes `users.email` to nullable (email-absent subjects store `email` NULL; `UNIQUE(email)` intact, NULLs distinct). Disputes stay anonymous (`raised_by` NULL); save/workspace flows stay subject-keyed and invoke no sync. Invariant gate untouched; `LocalUser` carries no verdict or creator rating. See "Supabase user sync" in the debt ledger.
...
- The three **intervention-and-scale** capabilities ship **dark by default** and are gated by a per-capability `Trust_Gate` (`core/trustGate.ts`): Feed Friction Dial (`GET /api/v1/friction` + the `app/apps/web/src/extension/` friction module), the Institutional GraphQL API (`POST /api/v1/graphql` behind `apiKeyAuth` + per-key rate limit), and creator coaching (`POST /api/v1/coaching`). Each requires Citation_Coverage **and** Model_Human_Agreement above their configured thresholds **and** a legal-review flag set true — `TRUST_{FEED,API,COACH}_{COVERAGE,AGREEMENT}_MIN` + `_LEGAL_OK` in `.env` (default `0.0`/`false`, so every capability is 503 until explicitly enabled). The gate is re-evaluated live per request (no redeploy to flip), fail-closed on any undefined metric. Every outbound payload passes `neutralityGuard` via `sendNeutral` (`http/respond.ts`); migration `008` is additive; `core/coaching.ts` holds no persistence handle (drafts are never stored). Invariant gate untouched and CI-guarded (`test/invariant.diffGuard.test.ts` fails the build if `core/assemble.ts` or `pipeline/stages.ts` is touched). See "Intervention & scale" in the debt ledger.
